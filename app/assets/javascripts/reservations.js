...
function show_rooms(){
  ...
  $('.room_check, .group_check').each(function(){
    ...
    if ( $(this).is(':checked') ){
      datepicker1.datepicker( {format: "dd/mm/yyyy", language: "es", autoclose: true, todayHighlight: true, todayBtn: 'linked' } ).on('changeDate', function(ev){ calculate_total_amount(); });;
      datepicker2.datepicker( {format: "dd/mm/yyyy", language: "es", autoclose: true, todayHighlight: true, todayBtn: 'linked' } ).on('changeDate', function(ev){ calculate_total_amount(); });;
